...
// withdrawalEngine.js
// Assumes global access to data.js variables: rrifLifMinimumRates, rrifLifMinRateAge95Plus, 
// ontarioLifMaximumFactors, lifMaxFactorAge90Plus_ON, bcLifMaximumFactors, lifMaxFactorAge95Plus_BC,
// albertaLifMaximumFactors, lifMaxFactorAge90Plus_AB

/** Step 4: Perform Withdrawals (Household Need, Individual Pools, Sequential) */
// *** MODIFICATION START: Accept province argument ***
function step4_PerformWithdrawals(yearData, currentAssets_user, currentAssets_spouse, strategy, userAge, spouseAge, province) {
// *** MODIFICATION END ***
    ...
    // *** MODIFICATION START: Pass province to helper function ***
    const maxAllowedLifUser = (yearData.openingBalance_user.lif || 0) * getLifMaximumFactor(userAge, province);
    const maxAllowedLifSpouse = hasSpouse ? ((yearData.openingBalance_spouse.lif || 0) * getLifMaximumFactor(spouseAge, province)) : 0;
    // *** MODIFICATION END ***
...
/**
 * Helper: Get Provincial LIF Maximum Withdrawal Factor from data.js
 * @param {number} age - The individual's age.
 * @param {string} province - The province (e.g., 'ON', 'BC').
 * @returns {number} The maximum withdrawal factor (e.g., 1/(90-age) or specific factor).
 */
// *** MODIFICATION START: Accept province and add provincial logic ***
function getLifMaximumFactor(age, province) {
    // Assuming min age for LIF is 55 based on data.js table
    if (age < 55) return 0;

    let factorsTable;
    let factorAge90Plus; // Use 95+ for BC, 90+ for AB/ON

    if (province === 'BC') {
        factorsTable = typeof bcLifMaximumFactors !== 'undefined' ? bcLifMaximumFactors : [];
        factorAge90Plus = typeof lifMaxFactorAge95Plus_BC !== 'undefined' ? lifMaxFactorAge95Plus_BC : 0.20;
    } else if (province === 'AB') {
        // *** NEW LOGIC FOR AB ***
        factorsTable = typeof albertaLifMaximumFactors !== 'undefined' ? albertaLifMaximumFactors : [];
        factorAge90Plus = typeof lifMaxFactorAge90Plus_AB !== 'undefined' ? lifMaxFactorAge90Plus_AB : 1.00;
    } else {
        // Default to Ontario rules if province is 'ON' or not recognized
        factorsTable = typeof ontarioLifMaximumFactors !== 'undefined' ? ontarioLifMaximumFactors : [];
        factorAge90Plus = typeof lifMaxFactorAge90Plus_ON !== 'undefined' ? lifMaxFactorAge90Plus_ON : 0.20;
    }

    if (factorsTable.length === 0) return 0; // Safety check
    ...
    // If age is over the max in the table, use the 90+ (or 95+) constant
    const maxAgeInTable = Math.max(...factorsTable.map(d => d.age));
    if (age > maxAgeInTable) {
         return factorAge90Plus;
    }
