 ...
 * @version     4.3.1 (Tweak: Optimizer step increased from 10 (5%) to 50 (1%))
 * @file        engineCore.js
 * @created     2025-10-25
...
// *** MODIFICATION v4.0.0: Calculate expenses from 'otherIncomes' array using individual COLAs ***
function step3_CalculateExpenses(yearData, scenario, settings, lastYearTaxPayable) {
    ...
    allItems.forEach(item => {
        ...
        if (item.type === 'expense') {
            ...
            const itemOwnerBirthYear = item.owner === 'spouse'
                ? (scenario.spouse?.data?.birthYear || (scenario.user?.birthYear || 0) + 1)
                : (scenario.user?.birthYear || 0);

            // Check if the item is active for the current age
            if (itemOwnerAge >= item.startAge && itemOwnerAge <= item.endAge) {
                const itemStartYear = itemOwnerBirthYear + (item.startAge || 0);
                // Years from baseYear to the year the expense starts
                ...
                // Years elapsed since the expense started
                const yearsSinceItemStart = Math.max(0, currentYear - itemStartYear);

                // Calculate the expense amount for the current year:
                // 1. Inflate the PV amount from baseYear to itemStartYear using GLOBAL COLA (consistent with income calc)
                ...
                const currentYearExpenseAmount = baseAmountAtStartYear * Math.pow(1 + itemColaRate, yearsSinceItemStart);
...
function _calculateOptimalSplit(yearData, scenario, settings, 
    ...
    // *** MODIFICATION v4.3.2: Change step from 10 (5%) to 500 (0.1%) ***
    const numSteps = 500; 
