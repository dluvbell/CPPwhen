 ...
 * @version     2.4.2 (Fix: Ensure welcome modal closes correctly by selecting elements by class/query)
 * @file        uiCore.js
 * @created     2025-10-25
...
const translations = {
    en: {
        ...
        legendAssumptions: "Global Assumptions", colaLabel: "Cost of Living Adj. (COLA, %)", lifeExpectancyLabel: "Max Calculation Age",
        ...
        legendGrowth: "Account Growth Rates (%)", returnRRSP: "RRSP/RRIF", returnTFSA: "TFSA", returnNonReg: "Non-Registered", returnLIF: "LIF",
        ...
        section2Title: "2. Analysis Results", loadingText: "Calculating...", toggleTableBtn: "Show/Hide Detailed Data", exportCsvBtn: "Export CSV",
    ...
    ko: { // Korean translations including COLA labels
        ...
        legendAssumptions: "공통 가정", colaLabel: "물가상승률 (COLA, %)", lifeExpectancyLabel: "최대 계산 나이",
        ...
        legendGrowth: "계좌별 성장률 (%)", returnRRSP: "RRSP/RRIF", returnTFSA: "TFSA", returnNonReg: "비등록", returnLIF: "LIF",
        ...
        section2Title: "2. 분석 결과", loadingText: "계산 중...", toggleTableBtn: "상세 데이터 보기/숨기기", exportCsvBtn: "CSV 저장",
...
function initializeCore() {
    ...
    // *** MODIFICATION START v2.4.2: Removed 'agreement-section', 'tab-nav' from ID list ***
    const allElementIds = [
        ...
        // Common/Result
        'runAnalysisBtn', 'loading-indicator', 'results-container', 'break-even-text-result', 'additional-metrics-container', 'toggle-details-btn', 'detailed-table-container', 'export-csv-btn',
        'welcome-modal', 'disclaimer-agree', 'agree-btn'
    ];
    // *** MODIFICATION END v2.4.2 ***
     ...
     // *** MODIFICATION START v2.4.2: Select elements without IDs correctly ***
     ...
     elements.agreement_section = document.querySelector('#welcome-modal .agreement-section'); // Select agreement section by class
     elements.tab_nav = document.querySelector('.tab-nav'); // Select tab nav by class
     // *** MODIFICATION END v2.4.2 ***
    ...
    // Initialize dependent modules (Ensure functions exist before calling)
    ...
    setLanguage(savedLang); // This must run after dependent modules are initialized if it calls their functions
    if(elements.welcome_modal) elements.welcome_modal.classList.remove('hidden');
}

// --- Core UI Functions ---
function handleWelcomeModalClose() {
    // *** MODIFICATION START v2.4.2: Ensure agreement_section is checked before using ***
...
function setLanguage(lang) {
    ...
    document.querySelectorAll('[data-lang-key]').forEach(el => {
        ...
        if (typeof translation === 'function') { el.textContent = translation({}); } // Handle dynamic text if needed
        else if (translation && typeof translation === 'string') {
            if (key === 'resultsP1' || key === 'resultsP2') { el.innerHTML = translation; } // Handle HTML content
            else if (key === 'createdBy') { if (el.childNodes.length > 0) el.childNodes[0].nodeValue = translation; } // Handle specific text node update
    ...
    if(elements.lang_toggle) elements.lang_toggle.textContent = translations[lang].langToggle;
    if(elements.modal_lang_toggle) elements.modal_lang_toggle.textContent = translations[lang].langToggle;

    // Trigger updates in other modules that depend on language (check if functions exist)
    ...
    if (typeof displayComparisonMetrics === 'function' && typeof getLastResultDetails === 'function' && getLastResultDetails()) {
        displayComparisonMetrics(getLastResultDetails());
    }
    if (typeof displayComparisonDetailedTable === 'function' && typeof getLastResultDetails === 'function' && getLastResultDetails()) {
...
function switchTab(tabName) {
    ...
    const targetPane = document.getElementById(`${tabName}-pane`); // Find pane by ID
...
// *** MODIFICATION START v2.4.1: Moved Helper Functions Here ***
/**
