 ...
 * @version     2.4.5 (Feat: Add translation for combined fieldset legend)
 * @file        uiCore.js
 * @created     2025-10-25
...
const translations = {
    ...
    ko: {
        ...
        legendAssumptions: "공통 가정", colaLabel: "전체 물가상승률 (%)", lifeExpectancyLabel: "최대 계산 나이", // Updated colaLabel
        legendGrowth: "계좌별 성장률 (%)", // Kept
        legendGrowthAssumptionsIncome: "성장률, 가정치 & 기타 소득", // *** ADDED v2.4.5 ***
        ...
        returnRRSP: "RRSP/RRIF (%)", returnTFSA: "TFSA (%)", returnNonReg: "비등록 (%)", returnLIF: "LIF (%)", // Added (%)
    ...
    document.querySelectorAll('[data-lang-key]').forEach(el => {
        ...
        if (translation !== undefined && translation !== null) {
            if (typeof translation === 'function') { el.textContent = translation({}); }
             ...
             // Optional: Log missing translations during development
             // console.warn(`Missing translation for key: ${key} in language: ${lang}`);
        }
    });
    document.querySelectorAll('[data-lang-key-tooltip]').forEach(el => {
        const key = el.getAttribute('data-lang-key-tooltip');
        if (translations[lang] && translations[lang][key]) { // Check language exists first
            el.setAttribute('data-tooltip', translations[lang][key]);
        }
    });
    if(elements.lang_toggle) elements.lang_toggle.textContent = translations[lang]?.langToggle || 'Lang'; // Fallback text
    if(elements.modal_lang_toggle) elements.modal_lang_toggle.textContent = translations[lang]?.langToggle || 'Lang'; // Fallback text
