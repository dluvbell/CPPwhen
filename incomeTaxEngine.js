 ...
 * @version     2.3.1 (Fix: Correct GIS income test and ON Surtax COLA)
...
// *** MODIFICATION START v3.3.0: Add pension splitting parameters with defaults ***
...
// *** MODIFICATION END v3.3.0 ***
    ...
    // *** MODIFICATION START v3.3.0: Apply pension split to net incomes ***
    // (Note: This is simplified. Real T1 applies split *after* Net Income calc, but this is functionally similar for taxable income)
    const pensionReceived_user = pensionTransfer_spouse;
    const pensionReceived_spouse = pensionTransfer_user;

    netIncome_user = netIncome_user - pensionTransfer_user + pensionReceived_user;
    if (hasSpouse) {
        netIncome_spouse = netIncome_spouse - pensionTransfer_spouse + pensionReceived_spouse;
    }
    // *** MODIFICATION END v3.3.0 ***
    ...
    // *** MODIFICATION START v3.3.0: Pass pension received amount to tax calculator ***
    const federalTax_user = calculateSingleTax(taxableIncome_user, userAge, adjustedFedBrackets, adjustedFedCredits, yearData.income.user, currentYear, province, settings, pensionReceived_user);
    const provincialTax_user = calculateSingleTax(taxableIncome_user, userAge, adjustedProvBrackets, adjustedProvCredits, yearData.income.user, currentYear, province, settings, pensionReceived_user);
    const tax_user = federalTax_user + provincialTax_user;

    let tax_spouse = 0;
    if (hasSpouse) {
        const federalTax_spouse = calculateSingleTax(taxableIncome_spouse, spouseAge, adjustedFedBrackets, adjustedFedCredits, yearData.income.spouse, currentYear, province, settings, pensionReceived_spouse);
        const provincialTax_spouse = calculateSingleTax(taxableIncome_spouse, spouseAge, adjustedProvBrackets, adjustedProvCredits, yearData.income.spouse, currentYear, province, settings, pensionReceived_spouse);
    ...
    const netIncomeForGis_user =
        (yearData.income.user.cpp || 0) +
        oasBeforeClawback_user +
        // (yearData.income.user.gis || 0) + // GIS is not included in income for *determining* next year's GIS
        (yearData.income.user.other || 0) +
        yearData.income.user.taxableWithdrawals +
        taxableNonRegGains_user;
    let netIncomeForGis_spouse = 0;
     if (hasSpouse) {
         netIncomeForGis_spouse =
            (yearData.income.spouse.cpp || 0) +
            oasBeforeClawback_spouse +
            // (yearData.income.spouse.gis || 0) + // GIS is not included in income for *determining* next year's GIS
            (yearData.income.spouse.other || 0) +
            yearData.income.spouse.taxableWithdrawals +
            taxableNonRegGains_spouse;
     }
...
// *** MODIFICATION START v3.3.0: Add pension income received parameter with default ***
function calculateSingleTax(taxableIncome, age, adjustedBrackets, adjustedCredits, incomeDetails, currentYear, province, settings, pensionIncomeReceived = 0) { // Added province for surtax check
// *** MODIFICATION END v3.3.0 ***
    ...
    if (age >= 65) {
        ...
        // *** MODIFICATION START v3.3.0: Add received pension income to credit calculation ***
    ...
    if (province === 'ON' && taxData.ON.surtax) {
        ...
        const colaMultiplierSurtax = Math.pow(1 + (settings?.cola || 0.025), yearsSinceBaseSurtax); // Use global settings COLA, fallback to 2.5%
