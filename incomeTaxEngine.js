...
function step5_CalculateTaxes(yearData, scenario, settings, currentUnrealizedGains_NonReg_user, currentUnrealizedGains_NonReg_spouse) {
    ...
    const federalTax_user = calculateSingleTax(taxableIncome_user, userAge, adjustedFedBrackets, adjustedFedCredits, yearData.income.user, currentYear, province, settings);
    const provincialTax_user = calculateSingleTax(taxableIncome_user, userAge, adjustedProvBrackets, adjustedProvCredits, yearData.income.user, currentYear, province, settings);
    const tax_user = federalTax_user + provincialTax_user;

    let tax_spouse = 0;
    if (hasSpouse) {
        const federalTax_spouse = calculateSingleTax(taxableIncome_spouse, spouseAge, adjustedFedBrackets, adjustedFedCredits, yearData.income.spouse, currentYear, province, settings);
        const provincialTax_spouse = calculateSingleTax(taxableIncome_spouse, spouseAge, adjustedProvBrackets, adjustedProvCredits, yearData.income.spouse, currentYear, province, settings);
...
function calculateSingleTax(taxableIncome, age, adjustedBrackets, adjustedCredits, incomeDetails, currentYear, province, settings) { // Added province for surtax check
